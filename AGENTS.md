...
- Claude is used in a two-model operator/reviewer split: Sonnet is the operator (generates render prompts and field suggestions, high frequency); Opus is the reviewer (gates the prompt before Gemini and reviews the rendered image after). Opus is the gatekeeper at both boundaries.
- `lib/claude/` exposes four functions: `buildRenderPromptRequest` and `buildSuggestFieldRequest` (Sonnet) plus `buildPromptReviewRequest` and `buildRenderReviewRequest` (Opus). Prompt review returns a structured verdict of `ship_it | revise | regenerate` with an issues array and an optional `revised_prompt`.
...
- Next.js scaffold target is Next.js 14 App Router with TypeScript strict, Tailwind, and shadcn/ui (new-york style). Required deps include `@supabase/supabase-js`, `@supabase/ssr`, `@google/genai`, `@anthropic-ai/sdk`, `zod`, `lucide-react`, `react-hook-form`, `@hookform/resolvers`, and `date-fns`.
- Phase 1 Supabase schema lives in `supabase/migrations/001_init.sql` with tables `properties`, `property_photos`, `rooms`, `room_specs`, `renders`, and `reference_materials`; Supabase clients are `lib/supabase/client.ts` and `lib/supabase/server.ts` per `@supabase/ssr`.
