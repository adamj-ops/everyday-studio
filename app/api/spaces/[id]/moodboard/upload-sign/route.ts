...
// Per-category hard cap. Count comes from the latest space_briefs row's
...
export async function POST(
  ...
  // Load the space to resolve its property_id and confirm RLS visibility.
  const { data: space, error: spaceErr } = await supabase
    ...
    .maybeSingle();
  if (spaceErr) {
    return internalError("moodboard_upload_sign_space", spaceErr);
  }
  if (!space) {
  ...
  const storagePath = `${space.property_id}/${spaceId}/${parsed.data.category_key}/${randomUUID()}.${extFromMime(parsed.data.mime_type)}`;
